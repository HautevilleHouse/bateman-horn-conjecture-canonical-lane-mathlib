...
/-!
# Source-derived formalization layer for `bateman-horn-conjecture-canonical-lane`
...
def sourceFormulaModels : List SourceFormulaModel := [
  { group := "constants", key := "eps_coh", status := "derived_numeric", formula := "eps_coh_raw", expr := (FormulaExpr.var "eps_coh_raw"), parseStatus := "parsed_source_expression", sourceSection := "paper/BATEMAN_HORN_CONJECTURE_PREPRINT.md Appendix E.6", notes := "Strict coherence target.", validation := "required_nonnegative", componentKeys := ["eps_coh_raw"], components := [
    { key := "eps_coh_raw", value := "0.0" }
  ] },
  { group := "constants", key := "horn_transfer", status := "derived_numeric", formula := "c_transfer_raw * transfer_gain_raw - e_transfer_raw", expr := (FormulaExpr.sub (FormulaExpr.mul (FormulaExpr.var "c_transfer_raw") (FormulaExpr.var "transfer_gain_raw")) (FormulaExpr.var "e_transfer_raw")), parseStatus := "parsed_source_expression", sourceSection := "paper/BATEMAN_HORN_CONJECTURE_PREPRINT.md Section 6.2 / Appendix E.4", notes := "horn_transfer theorem-level floor.", validation := "required_positive", componentKeys := ["c_transfer_raw", "e_transfer_raw", "transfer_gain_raw"], components := [
    { key := "c_transfer_raw", value := "1.117" },
    { key := "e_transfer_raw", value := "0.121" },
    { key := "transfer_gain_raw", value := "1.03" }
  ] },
  { group := "constants", key := "kappa_compact", status := "derived_numeric", formula := "1.0 / (1.0 + delta_comp_sup_raw)", expr := (FormulaExpr.div (FormulaExpr.num "1.0") (FormulaExpr.add (FormulaExpr.num "1.0") (FormulaExpr.var "delta_comp_sup_raw"))), parseStatus := "parsed_source_expression", sourceSection := "paper/BATEMAN_HORN_CONJECTURE_PREPRINT.md Section 5.3 / Appendix C", notes := "kappa_compact theorem-level floor.", validation := "required_positive", componentKeys := ["delta_comp_sup_raw"], components := [
    { key := "delta_comp_sup_raw", value := "0.246" }
  ] },
  { group := "constants", key := "kappa_density", status := "derived_numeric", formula := "c_density_raw * density_gain_raw - e_density_raw", expr := (FormulaExpr.sub (FormulaExpr.mul (FormulaExpr.var "c_density_raw") (FormulaExpr.var "density_gain_raw")) (FormulaExpr.var "e_density_raw")), parseStatus := "parsed_source_expression", sourceSection := "paper/BATEMAN_HORN_CONJECTURE_PREPRINT.md Section 4.5 / Appendix A", notes := "kappa_density theorem-level floor.", validation := "required_positive", componentKeys := ["c_density_raw", "density_gain_raw", "e_density_raw"], components := [
    { key := "c_density_raw", value := "1.447" },
    { key := "density_gain_raw", value := "0.918" },
    { key := "e_density_raw", value := "0.235" }
  ] },
  { group := "constants", key := "rho_rigidity", status := "derived_numeric", formula := "rho_rigidity_raw", expr := (FormulaExpr.var "rho_rigidity_raw"), parseStatus := "parsed_source_expression", sourceSection := "paper/BATEMAN_HORN_CONJECTURE_PREPRINT.md Section 6.1 / Appendix D", notes := "rho_rigidity theorem-level floor.", validation := "required_positive", componentKeys := ["rho_rigidity_raw"], components := [
    { key := "rho_rigidity_raw", value := "1.078" }
  ] },
  { group := "constants", key := "sigma_pattern", status := "derived_numeric", formula := "pattern_floor_raw - singular_loss_raw - restart_loss_raw", expr := (FormulaExpr.sub (FormulaExpr.sub (FormulaExpr.var "pattern_floor_raw") (FormulaExpr.var "singular_loss_raw")) (FormulaExpr.var "restart_loss_raw")), parseStatus := "parsed_source_expression", sourceSection := "paper/BATEMAN_HORN_CONJECTURE_PREPRINT.md Section 5.2 / Appendix B", notes := "sigma_pattern theorem-level floor.", validation := "required_positive", componentKeys := ["pattern_floor_raw", "restart_loss_raw", "singular_loss_raw"], components := [
    { key := "pattern_floor_raw", value := "1.392" },
    { key := "restart_loss_raw", value := "0.149" },
    { key := "singular_loss_raw", value := "0.169" }
  ] },
  { group := "stitch", key := "sigma_star_can", status := "derived_numeric", formula := "sigma_star_can_raw", expr := (FormulaExpr.var "sigma_star_can_raw"), parseStatus := "parsed_source_expression", sourceSection := "paper/BATEMAN_HORN_CONJECTURE_PREPRINT.md Section 5 / stitch constants", notes := "sigma_star_can theorem-level stitch constant.", validation := "required_positive", componentKeys := ["sigma_star_can_raw"], components := [
    { key := "sigma_star_can_raw", value := "1.054" }
  ] }
]

def formalizationCertificate : FormalizationCertificate := {
  sourceRepo := "bateman-horn-conjecture-canonical-lane",
